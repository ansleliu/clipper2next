...
auto execute_offset_request(const clipper2next::Paths64& paths,
                            ...
                            double arc_tolerance = 0.0) -> clipper2next::Paths64 {
...
auto BM_product_offset_round(benchmark::State& state) -> void {
    ...
    for (auto _ : state) {
        auto result = execute_offset_request(
            subjects, 18.0, clipper2next::JoinType::Round, clipper2next::EndType::Polygon, 2.0, 8U);
        benchmark::DoNotOptimize(result.data());
        benchmark::DoNotOptimize(result.size());
    }
...
auto BM_product_borrowed_offset_flat(benchmark::State& state) -> void {
    const auto subjects = product::make_offset_subjects(static_cast<std::size_t>(state.range(0)));
    auto request_group = clipper2next::borrowed_offset_group64{};
    auto request = clipper2next::borrowed_offset_request64{};
    request.groups = std::span{&request_group, 1U};
    request_group.paths = clipper2next::borrow_paths64(subjects);
    request.delta = 18.0;
    request_group.join_type = clipper2next::JoinType::Miter;
    request_group.end_type = clipper2next::EndType::Polygon;
...
BENCHMARK(BM_product_borrowed_offset_flat)->Arg(1)->Arg(2)->Arg(4)->Arg(8)->Arg(16)->Arg(64);
